...
serve(async (req) => {
  ...
  try {
    ...
    if (source === "smartlead") {
      ...
      if (!reply.smartlead_campaign_id || !reply.smartlead_stats_id) {
        await supabase.from("send_attempts").insert({
          reply_id,
          draft_version_id: draft.id,
          provider: "smartlead",
          success: false,
          request_payload: { error: "Missing smartlead_campaign_id or smartlead_stats_id" },
        });
        await supabase.from("inbound_replies").update({
          status: "failed",
          processing_error: "Missing SmartLead thread context (stats_id). Cannot reply.",
        }).eq("id", reply_id);
      ...
      sendPayload = {
        email_stats_id: reply.smartlead_stats_id,
        email_body: fullHtml,
        reply_message_id: reply.smartlead_reply_message_id || undefined,
        reply_email_time: reply.smartlead_reply_time || undefined,
      };
